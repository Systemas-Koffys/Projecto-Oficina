...
// URL del backend (Configurable para producci├│n o proxy local)
const API_URL = import.meta.env.VITE_API_URL || '/api';
console.log("DEBUG: La URL de la API es:", API_URL);

// Estado global para los cat├ílogos y solicitudes
...
// Funci├│n para descargar los cat├ílogos de MySQL
export async function fetchCatalogos() {
  ...
  try {
    ...
    // Poblar el store con los cat├ílogos principales
    ...
    console.log("Cat├ílogos base cargados desde MySQL.");
  } catch (error) {
    console.error("Error al cargar los cat├ílogos:", error);
    uiState.isLoading = false;
  }
}

// --- CONFIGURACI├ôN DEL SISTEMA ---
...
// Funci├│n para descargar historial de impresiones
...
// Funci├│n para registrar una impresi├│n (Individual o Hoja de Ruta)
...
export async function deleteImpresion(id) {
  try {
    ...
    console.error("Error al eliminar impresi├│n:", error);
    return false;
  }
}
...
// Funci├│n para descargar los usuarios p├║blicos (para el login)
export async function fetchPublicUsuarios() {
  try {
    const response = await fetch(`${API_URL}/usuarios/publico`);
    return await response.json();
  } catch (error) {
    console.error("Error al cargar usuarios p├║blicos:", error);
    return [];
  }
}

// Funci├│n para descargar los usuarios de MySQL
export async function fetchUsuarios() {
  try {
    const response = await fetch(`${API_URL}/usuarios`);
    store.usuarios = await response.json();
    console.log("Usuarios cargados desde MySQL con ├®xito.");
  } catch (error) {
...
// Funci├│n para descargar las solicitudes de MySQL
export async function fetchSolicitudes() {
  try {
    const response = await fetch(`${API_URL}/solicitudes`);
    store.solicitudes = await response.json();
    console.log("Solicitudes cargadas desde MySQL con ├®xito.");
  } catch (error) {
    console.error("Error al cargar las solicitudes:", error);
  }
}

// Funci├│n para guardar una nueva solicitud en MySQL
export async function addSolicitud(solicitud) {
  try {
    ...
    return { success: false, error: 'Error de conexi├│n: El servidor no responde' };
  }
}

// Funci├│n para ELIMINAR una solicitud de MySQL
...
// Funci├│n para actualizar una solicitud en MySQL
export async function updateSolicitud(id, datos) {
  try {
    ...
    return { success: false, error: 'Error de conexi├│n al actualizar' };
  }
}
// --- GESTI├ôN DE CAT├üLOGOS (DIN├üMICO) ---
